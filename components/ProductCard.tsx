    ...
    <Link href={`/products/${product.slug}`} className="card group block">
      ...
      <div className="aspect-square bg-gradient-to-br from-brand-green-bright/10 to-brand-green-pale flex items-center justify-center overflow-hidden relative">
        {product.coverImage ? (
          ...
          <img src={product.coverImage} alt={product.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
        ) : (
      ...
      <div className="p-5">
        ...
        {/* Excerpt */}
        {'excerpt' in product && product.excerpt && (
          <p className="text-gray-500 text-sm leading-relaxed line-clamp-2 mb-3">
            {(product as Product & { excerpt?: string }).excerpt}
          </p>
        )}
